...
/**
 * Write your backing code using Javascript via Nashorn.
 */
public class NashornObjectHandler extends ReflectionObjectHandler {
  private final ScriptEngine engine;

  public NashornObjectHandler(ScriptEngine engine) {
    this.engine = engine;
  }
...
//    }
  ...
  protected Wrapper findWrapper(final int scopeIndex, final Wrapper[] wrappers, final List<Guard> guards, final Object scope, final String name) {
    if (scope instanceof JSObject) {
      JSObject jso = (JSObject) scope;
      try{
        Object eval = jso.eval(name);
